...
const EditUser = () => {
    ...
    const handleSubmit = (e) => {
        ...
        axios
            .post(`http://localhost:8080/user/${id}`, updatedUser, {
                headers: {
    ...
    return (
        <>
            <div className="bg-black text-white p-8 mx-auto">
                <h1 className="text-3xl font-semibold mb-4 ml-96">Add {user.username}</h1>
                {message && (
                    <div
                        className={`text-white ${
                            message === 'User added successfully!' ? 'ml-40 bg-green-500 w-1/6' : ' ml-40 bg-red-500 w-1/6'
                        } p-2 rounded-md mb-4`}
                    >
                        {message}
                    </div>
